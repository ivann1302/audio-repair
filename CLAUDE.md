...
После завершения любого пункта из `PLAN.md`:
1. Отметь его как выполненный: `[ ]` → `[x]`, `[~]` → `[x]`
2. В конце `PLAN.md` обнови или добавь раздел `## Текущий статус` — одна-две строки: на чём остановились и что следующее
